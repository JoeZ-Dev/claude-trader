...
**`momentum_monitor` specifically uses a Schwab credential with real
trading capability** (see specs.md §4 for why - a separate data-only app
was not achievable). This makes the following a hard rule, not a
preference: `momentum_monitor` must contain NO code that calls any Schwab
order-placement, account-balance, or position endpoint. If a future task
would require this (an execution phase, if one is ever built), stop and
require the human to explicitly revisit specs.md §4 first - do not add
this code as an incidental part of a broader task.
# Agent Protocol — claude-trader
